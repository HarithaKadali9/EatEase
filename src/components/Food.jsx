                ...
                {products.map(product=>{
                    if(cartItems[product.id]!=0){
                        return <div key={product.id} className="glass my-3 grid grid-cols-5 overflow-hidden">
                                    ...
                                    <div>
                                        <p className="text-xl font-bold pl-2">{cartItems[product.id]}</p>
                                    ...
                                    <div className="absolute right-0 bottom-0 gap-2 font-bold">
                                        <button onClick={()=>removeFromCart(product.id)} className="text-red-600 bg-red-300 hover:text-red-100 p-2 rounded">Remove</button>
                                    </div>
                                </div>
                    }
                })}
                    ...
                    {products.map(product=>(
                        <div key={product.id} className="w-[380px] p-5 bg-white rounded-lg glass transition-all duration-200 hover:scale-110">
                            ...
                            <div className="flex flex-row justify-between items-center mt-5 gap-3">
                                ...
                                <button onClick={()=>{addToCart(product.id)}} className="p-3 text-2xl w-14 rounded-xl bg-orange-400 hover:bg-orange-800
                                "><BsCart className="mx-auto"/></button>
                                {cartItems[product.id]>0 && <div className="absolute flex items-center justify-center top-5 left-5 bg-green-600 font-bold rounded-lg h-12 w-12">{cartItems[product.id]}s</div>}
                            </div>
                        </div>
                    ))}
                </div>
